class DAOUser{
	...
	findByName(name,currentUserId,callback){
		...
		if(err) callback(new Error("Error al obtener la conexion"), null);
			...
			connection.query("select * from (select name,id,status from users left join (select * from friendrequests where originUser = ? or destinationUser= ?) as rel on (users.id = rel.originUser or users.id = rel.destinationUser))as result where id != ? and name = ?;",
