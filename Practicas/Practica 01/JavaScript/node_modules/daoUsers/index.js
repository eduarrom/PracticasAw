		...
		this.pool.getConnection((err,connection)=>{
			...
			if(err){
				...
				connection.query("select * from users where id = ?",[id],(err,rows)=>{
					...
					if(err) callback(new Error("Error al obtener usuarios por id"),null);
	...
	getPendingFriendRequest(userId,callback){
		this.pool.getConnection((err,connection)=>{
			...
			if (err) callback(new Error("Error al obtener la conexion"));
				...
				connection.query("select id,name,image from users,friendrequests where status = 0 and destinationUser = ? and id = originUser;",[userId],
				(err,result)=>{
