class DAOUser{
	...
	findByName(name,currentUserId,callback){
		...
		if(err) callback(new Error("Error al obtener la conexion"), null);
			...
			connection.query("select name,id,status,points from users left join (select * from friendrequests where originUser = ? or destinationUser= ?) as req on (id=req.destinationUser or id=req.originuser) where originUser = ? or destinationUser = ? and id != ? and name = ?;",
			[currentUserId,currentUserId,currentUserId,currentUserId,currentUserId,name],(err,result)=>{
